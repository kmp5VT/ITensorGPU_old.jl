using ITensorGPU: ITensorGPU, CuArray, CuVector, CuDense
using ITensors: Index, cu, cpu, dim, tensor, randomITensor
using ITensors.NDTensors: Dense, data
...
@testset "cuITensor, Dense{$SType} storage" for SType in (Float64, ComplexF64)
  ...
  a = Index(ma, "a")
  indices = [i, j, k, l, a]
  ## TODO how do tensors add in NDTensors?
  #=
  @testset "Test add CuDense" begin
    A = [SType(1.0) for ii in 1:dim(i), jj in 1:dim(j)]
    dA = tensor(CuDense{SType,CuVector{SType,ITensorGPU.default_buffertype()}}(
      SType(1.0), dim(i) * dim(j)
    ), (i,j))
    B = [SType(2.0) for ii in 1:dim(i), jj in 1:dim(j)]
    dB = tensor(CuDense{SType,CuVector{SType,ITensorGPU.default_buffertype()}}(
  ...
  @testset "Test move CuDense on/off GPU" begin
    A = [SType(1.0) for ii in 1:dim(i), jj in 1:dim(j)]
    dA = CuDense{SType,CuVector{SType,ITensorGPU.default_buffertype()}}(
      Dense(vec(A))
    )
    dB = convert(Dense{SType,Vector{SType}}, dA)
    @test data(dB) == vec(A)
  end
  
  @testset "Test basic CuDense features" begin
    @test Dense{SType,CuVector{SType,ITensorGPU.default_buffertype()}}(10) isa
      CuDense{SType}
    @test complex(Dense{SType,CuVector{SType}}) ==
      Dense{complex(SType),CuVector{complex(SType)}}
  end
  
  if SType == Float64
    @testset "Test CuDense complex" begin
      A = cu(rand(SType, dim(i) * dim(j)))
      dA = CuDense{SType,CuVector{SType,ITensorGPU.default_buffertype()}}(A)
      dC = complex(dA)
